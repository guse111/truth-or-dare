...
class Database:
    ...
    def create_game(self, chat_id: int, creator_id: int, difficulty: str, context_tags: str) -> int:
        with get_db_connection() as conn:
            conn.execute('INSERT OR IGNORE INTO users (id) VALUES (?)', (creator_id,))
            cursor = conn.execute(
    ...
    def get_total_turns(self, game_id: int) -> int:
        with get_db_connection() as conn:
            result = conn.execute('SELECT total_turns FROM games WHERE id = ?', (game_id,)).fetchone()
            return result['total_turns'] if result else 0

    def get_player_count(self, game_id: int) -> int:
        with get_db_connection() as conn:
            result = conn.execute('SELECT COUNT(*) as count FROM game_players WHERE game_id = ?', (game_id,)).fetchone()
    ...
    def get_card(self, difficulty: str, card_type: str, context: str = None) -> Optional[sqlite3.Row]:
        with get_db_connection() as conn:
            ...
            if context and context != 'general':
                query += " AND (tags LIKE ? OR tags IS NULL)"
                params.append(f"%{context}%")  # ✅ Безопасная передача параметра
            
            query += ' ORDER BY RANDOM() LIMIT 1'
            result = cursor.execute(query, params).fetchone()
            
            if result:
                cursor.execute('UPDATE cards SET usage_count = usage_count + 1 WHERE id = ?', (result['id'],))
            
            return result

    def get_all_cards(self):  # ✅ ИСПРАВЛЕНО: добавлен self, выровнены отступы
        with get_db_connection() as conn:
            return conn.execute('SELECT * FROM cards').fetchall()

    def add_card(self, text: str, card_type: str, difficulty: str, 
                 tags: str = None, source: str = 'manual', is_verified: int = 0):  # ✅ ИСПРАВЛЕНО
        with get_db_connection() as conn:
            conn.execute(
    ...
    def save_current_task(self, game_id: int, task_text: str, title: str, context: str, task_type: str):
        with get_db_connection() as conn:
            conn.execute(
                '''UPDATE games SET 
                   current_task_text = ?, current_task_title = ?, 
                   current_task_context = ?, current_task_type = ?
                   WHERE id = ?''',
    ...
    def clear_current_task(self, game_id: int):
        with get_db_connection() as conn:
            conn.execute(
                '''UPDATE games SET 
                   current_task_text = NULL, current_task_title = NULL, 
                   current_task_context = NULL, current_task_type = NULL
                   WHERE id = ?''',
                (game_id,)
            )
    ...
    def get_stats(self):
        with get_db_connection() as conn:
            ...
            # 2. Игры
            finished_games = cursor.execute("SELECT COUNT(*) FROM games WHERE status='finished'").fetchone()[0]
            active_games = cursor.execute("SELECT COUNT(*) FROM games WHERE status='active'").fetchone()[0]
            total_turns = cursor.execute("SELECT COALESCE(SUM(total_turns), 0) FROM games").fetchone()[0]
            
            # 3. Карточки
            total_cards = cursor.execute('SELECT COUNT(*) FROM cards').fetchone()[0]
            cards_by_type = {row['type']: row['count'] for row in cursor.execute(
                'SELECT type, COUNT(*) as count FROM cards GROUP BY type'
            ).fetchall()}
            
            top_card = cursor.execute(
                'SELECT text, type, difficulty, usage_count FROM cards ORDER BY usage_count DESC LIMIT 1'
            ).fetchone()
            
            return {
                'users': total_users,
                'games_finished': finished_games,
                'games_active': active_games,
                'turns_total': total_turns,
                'cards_total': total_cards,
                'cards_dare': cards_by_type.get('dare', 0),
                'cards_truth': cards_by_type.get('truth', 0),
                'top_card': top_card
            }
